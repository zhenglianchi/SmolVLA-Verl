...
exec vvla-train-sac \
  ...
  --config-name dsrl \
  output_dir=./outputs/rl/dsrl/pi05/libero-spatial-task2-cnn-noise05 \
  ...
  cluster.actor_rollout_ref.model.adapter.dsrl.rollout_noise_scale=0.5 \
  trainer.experiment_name=pi05-libero-spatial-task2-cnn-noise05-dsrl \
  ray_kwargs.ray_init.runtime_env.env_vars.MUJOCO_GL=osmesa \
  ray_kwargs.ray_init.runtime_env.env_vars.TENSORBOARD_DIR=./outputs/rl/dsrl/pi05/libero-spatial-task2-cnn-noise05/tensorboard \
  "$@"
