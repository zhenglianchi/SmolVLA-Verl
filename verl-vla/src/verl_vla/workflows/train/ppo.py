...
from verl_vla.trainer.ppo.rob_ray_trainer import RobRayPPOTrainer
...
def run_ppo(config):
    ...
    if (
        ...
        profiled_run_ppo = _run_ppo_remote.options(runtime_env={"nsight": nsight_options})
        result = ray.get(profiled_run_ppo.remote(config))
    else:
        result = ray.get(_run_ppo_remote.remote(config))
