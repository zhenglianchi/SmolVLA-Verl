...
# Vendored from LeRobot:
#   src/lerobot/policies/gaussian_actor/modeling_gaussian_actor.py
#   commit 22bd7a2f489b367d8df42de803b1e8c4ca63a3f9
# Functional changes are limited to the three import paths marked below so
# this source can run against verl-vla's pinned LeRobot release. The file is
# formatted with the repository's standard Ruff configuration.
# ruff: noqa: B008
...
from lerobot.policies.pretrained import PreTrainedPolicy  # verl-vla compatibility import
from lerobot.policies.utils import get_device_from_parameters  # verl-vla compatibility import
from lerobot.utils.constants import ACTION, OBS_ENV_STATE, OBS_STATE
from torch import Tensor
from torch.distributions import MultivariateNormal, TanhTransform, Transform, TransformedDistribution

from .configuration import GaussianActorConfig, is_image_feature  # verl-vla compatibility import
...
class GaussianActorPolicy(
    ...
    def get_optim_params(self) -> dict:
        optim_params = {
            "actor": [
                p for n, p in self.actor.named_parameters() if not n.startswith("encoder") or not self.shared_encoder
            ],
        }
        return optim_params
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        raise NotImplementedError("GaussianActorPolicy does not support action chunking. It returns single actions!")
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        if self.config.num_discrete_actions is not None:
            if self.discrete_critic is not None:
                ...
                discrete_action = torch.ones((*actions.shape[:-1], 1), device=actions.device, dtype=actions.dtype)
...
class GaussianActorObservationEncoder(nn.Module):
    ...
    def forward(self, obs: dict[str, Tensor], cache: dict[str, Tensor] | None = None, detach: bool = False) -> Tensor:
        parts = []
        if self.has_images:
            if cache is None:
        ...
        raise ValueError("No parts to concatenate, you should have at least one image or environment state or state")
...
class DiscreteCritic(nn.Module):
    ...
    def forward(self, observations: torch.Tensor, observation_features: torch.Tensor | None = None) -> torch.Tensor:
